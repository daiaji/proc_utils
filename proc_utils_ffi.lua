...
-- refactored with an idiomatic Lua OOP wrapper.
-- Author: Gemini AI
-- Version: 2.3.5 (Fixed BOOL truthiness checks and CI layout)
...
ffi.cdef[[
    ...
    static const DWORD FORMAT_MESSAGE_ALLOCATE_BUFFER = 0x000100;
...
internal.wstr_to_str = wstr_to_str

local function str_to_wstr(str)
...
internal.str_to_wstr = str_to_wstr

local function formatWinError(err_code)
...
internal.formatWinError = formatWinError

function internal.forEachProcess(callback)
...
function internal.findProcess(name_or_pid)
    ...
    local found_pid = 0
    internal.forEachProcess(function(pe)
...
function internal.getProcessPath(pid, buffer, buffer_size, process_handle)
    ...
    -- [FIX] Explicit check against 0 for BOOL return
...
function internal.getProcessCommandLine(pid, buffer, buffer_size)
    buffer[0] = 0
...
function internal.terminateProcessTree(pid)
    internal.forEachProcess(function(pe)
        if pe.th32ParentProcessID == pid then
            internal.terminateProcessTree(pe.th32ProcessID)
        end
    end)
    C_API.ProcUtils_TerminateProcessByPid(pid, 1)
end

function C_API.ProcUtils_OpenProcessByPid(pid, desired_access)
...
function C_API.ProcUtils_TerminateProcessByPid(pid, exit_code)
    ...
    if not h_proc or h_proc == INVALID_HANDLE_VALUE then return false end
    local proc_handle = ffi.gc(h_proc, kernel32.CloseHandle)
    return kernel32.TerminateProcess(proc_handle, exit_code or 0)
end

-- Full C-API functions...
function C_API.ProcUtils_OpenProcessByName(process_name, desired_access)
    local pid = internal.findProcess(process_name)
    if pid == 0 then kernel32.SetLastError(C.ERROR_NOT_FOUND); return nil end
    return C_API.ProcUtils_OpenProcessByPid(pid, desired_access)
end
function C_API.ProcUtils_ProcessExists(process_name_or_pid)
    if not process_name_or_pid or process_name_or_pid == "" then kernel32.SetLastError(C.ERROR_INVALID_PARAMETER); return 0 end
    return internal.findProcess(process_name_or_pid)
end
function C_API.ProcUtils_FindAllProcesses(process_name, out_pids, pids_array_size)
...
function C_API.ProcUtils_CreateProcess(command, working_dir, show_mode, desktop_name)
    ...
    if desktop_name and desktop_name ~= "" then si.lpDesktop = internal.str_to_wstr(desktop_name) end
    local pi = ffi.new("PROCESS_INFORMATION")
    local cmd_buffer_w = internal.str_to_wstr(command)
    local wd_wstr = internal.str_to_wstr(working_dir)
    -- [FIX] Explicit check against 0 for BOOL return
...
function C_API.ProcUtils_LaunchProcess(command, working_dir, show_mode, desktop_name)
    local result = C_API.ProcUtils_CreateProcess(command, working_dir, show_mode, desktop_name)
    if result.pid == 0 then kernel32.SetLastError(result.last_error_code); return 0 end
    kernel32.CloseHandle(result.process_handle)
    return result.pid
end
function C_API.ProcUtils_CreateProcessAsSystem(command, working_dir, show_mode)
    local result = ffi.new("ProcUtils_ProcessResult")
    local last_error
    repeat
        ...
        local si = ffi.new("STARTUPINFOW"); si.cb = ffi.sizeof(si)
        si.dwFlags, si.wShowWindow = C.STARTF_USESHOWWINDOW, show_mode or _M.constants.SW_SHOW
        si.lpDesktop = internal.str_to_wstr("winsta0\\default")
        local pi = ffi.new("PROCESS_INFORMATION")
        local cmd_buffer_w = internal.str_to_wstr(command)
        local wd_wstr = internal.str_to_wstr(working_dir)
        -- [FIX] Explicit check against 0 for BOOL return
...
function C_API.ProcUtils_ProcessGetInfo(pid, out_info)
    ...
    local proc_handle = ffi.gc(h_proc, kernel32.CloseHandle)
    local found = false
    internal.forEachProcess(function(pe) if pe.th32ProcessID == pid then out_info.parent_pid, out_info.thread_count, found = pe.th32ParentProcessID, pe.cntThreads, true; return true end end)
    ...
    internal.getProcessPath(pid, out_info.exe_path, 260, proc_handle)
    internal.getProcessCommandLine(pid, out_info.command_line, 2048)
    out_info.pid = pid
    return true
end
function C_API.ProcUtils_ProcessGetCommandLine(pid, out_cmd, cmd_size)
    if pid == 0 or out_cmd == nil or cmd_size <= 0 then kernel32.SetLastError(C.ERROR_INVALID_PARAMETER); return false end
    return internal.getProcessCommandLine(pid, out_cmd, cmd_size)
end
function C_API.ProcUtils_ProcessGetPath(pid, out_path, path_size)
    if pid == 0 or out_path == nil or path_size <= 0 then kernel32.SetLastError(C.ERROR_INVALID_PARAMETER); return false end
    return internal.getProcessPath(pid, out_path, path_size, nil) > 0
end
function C_API.ProcUtils_ProcessGetParent(process_name_or_pid)
    local child_pid = internal.findProcess(process_name_or_pid); if not child_pid or child_pid == 0 then return 0 end
    local parent_pid = 0
    internal.forEachProcess(function(pe) if pe.th32ProcessID == child_pid then parent_pid = pe.th32ParentProcessID; return true end end)
    return parent_pid
end
function C_API.ProcUtils_ProcessClose(process_name_or_pid, exit_code)
    local pid = internal.findProcess(process_name_or_pid); if pid == 0 then kernel32.SetLastError(C.ERROR_NOT_FOUND); return false end
    return C_API.ProcUtils_TerminateProcessByPid(pid, exit_code)
end
function C_API.ProcUtils_ProcessCloseTree(process_name_or_pid)
    local pid = internal.findProcess(process_name_or_pid); if not pid or pid == 0 then kernel32.SetLastError(C.ERROR_NOT_FOUND); return false end
    return C_API.ProcUtils_TerminateProcessTreeByPid(pid)
end
function C_API.ProcUtils_TerminateProcessTreeByPid(pid)
    if pid == 0 then return true end; internal.terminateProcessTree(pid); return true
end
function C_API.ProcUtils_ProcessSetPriority(process_name_or_pid, priority)
    ...
    local pclass = pmap[priority:upper()]; if not pclass then kernel32.SetLastError(C.ERROR_INVALID_PARAMETER); return false end
    local pid = internal.findProcess(process_name_or_pid); if not pid or pid == 0 then kernel32.SetLastError(C.ERROR_NOT_FOUND); return false end
    local h_proc = kernel32.OpenProcess(C.PROCESS_SET_INFORMATION, false, pid)
    if not h_proc or h_proc == INVALID_HANDLE_VALUE then return false end
    local proc_handle = ffi.gc(h_proc, kernel32.CloseHandle)
    return kernel32.SetPriorityClass(proc_handle, pclass)
end
function C_API.ProcUtils_ProcessWait(process_name, timeout_ms)
    if not process_name or process_name == "" then kernel32.SetLastError(C.ERROR_INVALID_PARAMETER); return 0 end
    local start = ffi.C.GetTickCount64()
    while true do
        local pid = internal.findProcess(process_name); if pid > 0 then return pid end
        if timeout_ms >= 0 and (ffi.C.GetTickCount64() - start) >= timeout_ms then kernel32.SetLastError(C.WAIT_TIMEOUT); return 0 end
        ffi.C.Sleep(100)
    end
end
function C_API.ProcUtils_ProcessWaitClose(process_name_or_pid, timeout_ms)
    if not process_name_or_pid or process_name_or_pid == "" then kernel32.SetLastError(C.ERROR_INVALID_PARAMETER); return false end
    local pid = internal.findProcess(process_name_or_pid); if pid == 0 then return true end
    local h_proc = kernel32.OpenProcess(C.SYNCHRONIZE, false, pid)
    if not h_proc or h_proc == INVALID_HANDLE_VALUE then return false end
    local proc_handle = ffi.gc(h_proc, kernel32.CloseHandle)
    return kernel32.WaitForSingleObject(proc_handle, timeout_ms < 0 and C.INFINITE or timeout_ms) == C.WAIT_OBJECT_0
end
function C_API.ProcUtils_WaitForProcessExit(process_handle, timeout_ms)
...
-------------------------------------------------------------------------------
-- High-Level OOP Wrapper (Recommended API)
-------------------------------------------------------------------------------
...
function _M.exec(command, working_dir, show_mode, desktop_name)
    ...
    return nil, err_code, internal.formatWinError(err_code)
end

function _M.open_by_pid(pid, access)
    access = access or _M.constants.PROCESS_ALL_ACCESS
    local handle = C_API.ProcUtils_OpenProcessByPid(pid, access)
    if handle then
...
function _M.open_by_name(name, access)
    access = access or _M.constants.PROCESS_ALL_ACCESS
    local pid = C_API.ProcUtils_ProcessExists(name)
    if pid > 0 then
...
-- [FIXED] Check integer return against 0, not boolean 'not'
function Process:terminate(exit_code)
    if C_API.ProcUtils_TerminateProcessByPid(self.pid, exit_code or 0) == 0 then
        local err_code = kernel32.GetLastError()
        return nil, err_code, internal.formatWinError(err_code)
    end
    return true
...
function Process:terminate_tree()
    if not C_API.ProcUtils_TerminateProcessTreeByPid(self.pid) then
        local err_code = kernel32.GetLastError()
        return nil, err_code, internal.formatWinError(err_code)
    end
    return true
...
function Process:wait_for_exit(timeout_ms)
    return C_API.ProcUtils_WaitForProcessExit(self._handle, timeout_ms)
end

function Process:get_info()
    local info_cdata = ffi.new("ProcUtils_ProcessInfo")
    if C_API.ProcUtils_ProcessGetInfo(self.pid, info_cdata) then
        return {
            pid = info_cdata.pid,
...
function Process:get_path()
    ...
    if C_API.ProcUtils_ProcessGetPath(self.pid, buffer, 260) then
...
function Process:get_command_line()
    ...
    if C_API.ProcUtils_ProcessGetCommandLine(self.pid, buffer, 2048) then
        return wstr_to_str(buffer)
    end
    local err_code = kernel32.GetLastError()
    return nil, err_code, internal.formatWinError(err_code)
...
function _M.find_all(name)
    local count = C_API.ProcUtils_FindAllProcesses(name, nil, 0)
    if count < 0 then 
        local err_code = kernel32.GetLastError()
        return nil, err_code, internal.formatWinError(err_code)
    end
    ...
    local stored = C_API.ProcUtils_FindAllProcesses(name, buffer, count)
    local pids = {}
    for i = 0, stored - 1 do
        pids[i + 1] = buffer[i]
    end
...
function _M.exists(name_or_pid)
    return C_API.ProcUtils_ProcessExists(name_or_pid)
end

function _M.terminate_by_pid(pid, exit_code)
    return C_API.ProcUtils_TerminateProcessByPid(pid, exit_code or 0)
end
