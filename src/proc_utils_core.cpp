...
{
    if (g_procutils_should_exit.load(std::memory_order_relaxed)) return;
...
extern "C" {
    ...
    {
        if (!process_name_or_pid || !*process_name_or_pid) return 0;
    ...
    PROC_UTILS_API unsigned int ProcUtils_Exec(const wchar_t* command, const wchar_t* working_dir, int show_mode, bool wait, const wchar_t* desktop_name)
    {
        ...
        if (wait) {
            while (!g_procutils_should_exit.load(std::memory_order_relaxed)) {
                // 修正：将 ScopedHandle 转换为 HANDLE，再取其地址
    ...
    PROC_UTILS_API bool ProcUtils_ProcessSetPriority(const wchar_t* process_name_or_pid, wchar_t priority) {
        ...
        case L'L': priority_class = IDLE_PRIORITY_CLASS; break;
        case L'B': priority_class = BELOW_NORMAL_PRIORITY_CLASS; break;
        case L'N': priority_class = NORMAL_PRIORITY_CLASS; break;
        case L'A': priority_class = ABOVE_NORMAL_PRIORITY_CLASS; break;
        case L'H': priority_class = HIGH_PRIORITY_CLASS; break;
        case L'R': priority_class = REALTIME_PRIORITY_CLASS; break;
        ...
        if (!h_process.IsValid()) return false;
